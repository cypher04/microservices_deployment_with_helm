...
resource "azurerm_private_dns_zone_virtual_network_link" "postgresql_pdz_vnet_link" {
    name                  = "postgresqlpdzvnetlink"
    resource_group_name   = var.resource_group_name
    private_dns_zone_name = azurerm_private_dns_zone.postgresql_pdz.name
    virtual_network_id    = var.vnet_id
    depends_on = [ var.database_subnet]
}
